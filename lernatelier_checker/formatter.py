...
def _compute_checks(result: ComplianceResult) -> list[_CheckItem]:
    ...
    if not result.file_found:
        items.append(_CheckItem(_Severity.ERROR, "Datei-Struktur nicht erkannt", "File structure not recognized"))
        return items
    ...
    if n > 0:
        ...
        items.append(_CheckItem(_Severity.ERROR, "Keine Tageseinträge gefunden", "No daily entries found"))
...
def _render_linter(result: ComplianceResult, filename: str, items: list[_CheckItem], lang: str) -> str:
    errors = [item.message_de if lang == "de" else item.message_en
              for item in items if item.severity == _Severity.ERROR]
    warnings = [item.message_de if lang == "de" else item.message_en
                for item in items if item.severity == _Severity.WARNING]
    passes = [item.message_de if lang == "de" else item.message_en
              for item in items if item.severity == _Severity.OK]
    ...
    if lang == "de":
        ...
        phrase = {Status.RED: "\U0001f534 Handlungsbedarf", Status.YELLOW: "\U0001f7e1 Verbesserungsbedarf", Status.GREEN: "\U0001f7e2 Alles in Ordnung"}[status]
    else:
        count_str = f"{n_errors} error{'s' if n_errors != 1 else ''}, {n_warnings} warning{'s' if n_warnings != 1 else ''}"
        phrase = {Status.RED: "\U0001f534 action needed", Status.YELLOW: "\U0001f7e1 improvements needed", Status.GREEN: "\U0001f7e2 all good"}[status]
